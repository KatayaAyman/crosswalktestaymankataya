...
import { TagService } from '../tag/tag.service'; // Import TagService
...
export class ArticleService {
  ...
  async create(userId: number, dto: CreateArticleDto) {
    ...
    const newTags : string[] = dto.tagList.split(",");
    article.tagList.push(...newTags);
    user?.articles.add(article);
    const tags = await this.tagService.createTagsIfNotExist(newTags);
    await this.em.flush();
